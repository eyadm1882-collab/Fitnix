...
import {
  Dumbbell, Sparkles, Loader2, AlertCircle, Bot, UtensilsCrossed,
  Scan, Camera, CreditCard, Flame,
  Activity, Clock, CheckCircle2, CalendarDays, Apple
} from "lucide-react";
import ApprovalGate from "@/components/workout/ApprovalGate";
import WorkoutDayCard from "@/components/workout/WorkoutDayCard";

interface Exercise {
  name: string; muscleGroup: string; sets: string; reps: string;
}
interface WorkoutDay {
  day: string; focus: string; exercises: Exercise[];
}
interface Stats {
  usersCount: number; subscriptionsCount: number; pendingCount: number;
  activeSubscriptions: number; documentsCount: number; messagesCount: number;
}

const DAY_ORDER = ["السبت", "الأحد", "الاثنين", "الثلاثاء", "الأربعاء", "الخميس", "الجمعة"];

function getDefaultDays(): string[] {
  const today = new Date().getDay();
  const arabicDays = ["الأحد", "الاثنين", "الثلاثاء", "الأربعاء", "الخميس", "الجمعة", "السبت"];
  const start = (today + 1) % 7;
  return Array.from({ length: 4 }, (_, i) => arabicDays[(start + i) % 7]);
}
...
function DashboardContent() {
  ...
  useEffect(() => {
    ...
    const data = localStorage.getItem("fitnix_workout_data");
