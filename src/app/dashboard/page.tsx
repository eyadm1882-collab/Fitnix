  ...
  if (loading) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center">
        <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-fitnix/5">
          <Dumbbell className="h-8 w-8 text-fitnix animate-pulse" />
        </div>
        <p className="mt-4 text-sm text-gray-400">جاري تجهيز مركز القيادة...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center">
        ...
        <button onClick={generatePlan} className="btn-primary btn-sm">إعادة المحاولة</button>
      </div>
    );
  }
  ...
  return (
    <div className="container-premium pt-6 pb-12">
      <div className="mx-auto max-w-5xl">
        {/* Hero */}
        <motion.div
          className="relative mb-6 overflow-hidden rounded-2xl bg-gradient-to-br from-gray-900 via-gray-800 to-black p-6 sm:p-8"
        ...
        >
          ...
          <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-xs font-medium text-fitnix">Fitnix AI Command Center</p>
              <h1 className="mt-1 text-2xl font-black text-white sm:text-3xl">مرحباً، {userName} 👋</h1>
              <p className="mt-1 text-sm text-gray-400">خطة اليوم بانتظارك 💪</p>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              {goalLabel !== "---" && <span className="badge badge-green text-xs">{goalLabel}</span>}
              {locationLabel !== "---" && <span className="badge badge-gray text-xs">{locationLabel}</span>}
              <Link href="/ai" className="btn-primary btn-sm !rounded-lg">
          ...
          {quickActions.map((action, i) => (
            <Link key={action.label} href={action.href}>
              <motion.div
                ...
                whileHover={{ y: -2 }}
              >
                <div className={`mb-3 flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br ${action.color} shadow-sm transition-all group-hover:scale-110`}>
        ...
        {stats && (
          ...
          >
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-green-200/50">
              <CheckCircle2 className="h-5 w-5 text-green-600" />
            </div>
            <div className="flex-1">
              <p className="text-sm font-bold text-green-800">اشتراك نشط</p>
              <p className="text-xs text-green-600">{stats.activeSubscriptions} مشترك نشط</p>
            </div>
            <Link href="/admin/knowledge" className="btn-ghost btn-sm !text-green-700 !bg-green-100/50 hover:!bg-green-100">
              الإحصائيات
            </Link>
          </motion.div>
        )}
        ...
        {workout.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
            <div className="mb-4 flex items-center justify-between">
              ...
              <button onClick={generatePlan} className="btn-ghost btn-sm !text-fitnix">
