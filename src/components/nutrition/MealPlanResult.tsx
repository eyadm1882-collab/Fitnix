...
const mealIcons: Record<string, any> = {
  "الإفطار": Coffee, "وجبة ما قبل التمرين": Apple, "الغداء": Sun,
  "وجبة خفيفة": Apple, "العشاء": Moon, "وجبة قبل النوم": Moon, "وجبة خفيفة مسائية": Moon,
};
    ...
    <motion.div className="space-y-5" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          ...
          <motion.div key={stat.label} className="rounded-xl border border-gray-100 bg-white p-4 text-center shadow-sm"
      ...
      {/* Water */}
      <motion.div className="flex items-center gap-3 rounded-xl bg-blue-50 p-4" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
        <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-blue-200/50">
        ...
        <span className="mr-auto text-xl font-black text-blue-600">{(plan.waterMl / 1000).toFixed(1)}L</span>
      </motion.div>

      {/* Meals */}
      <div className="space-y-3">
        <h3 className="flex items-center gap-2 text-lg font-bold text-gray-900">
          <UtensilsCrossed className="h-5 w-5 text-fitnix" /> الوجبات
        </h3>
        {plan.meals.map((meal, mi) => {
          const Icon = mealIcons[meal.name] || UtensilsCrossed;
          return (
            <motion.div key={meal.name} className="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm"
              initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 + mi * 0.08 }}>
              <div className="flex items-center justify-between border-b border-gray-50 bg-gray-50/50 p-4">
                <div className="flex items-center gap-3">
                  <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-fitnix/10 to-fitnix/5">
      ...
      {/* Shopping List */}
      <motion.div className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm"
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          {plan.shoppingList.map((item, i) => (
            <motion.div key={i} className="flex items-center gap-2 rounded-xl bg-gray-50 px-4 py-2.5"
              initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.8 + i * 0.04 }}>
